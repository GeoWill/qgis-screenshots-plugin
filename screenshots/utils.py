...
def sessionDataFolder(sessionFolderName):
    """ Returns the folder where to store sessions data. It is created
    inside the screenshotsPluginBaseFolder().
    """
    folder = os.path.join(screenshotsPluginBaseFolder(), "data", sessionFolderName)
...
def screenshotsBaseFolder():
    """Returns the folder where to store sessions. It is created
    inside the screenshotsPluginBaseFolder().
    """
    folder = os.path.join(screenshotsPluginBaseFolder(), "sessions")
    if not QDir(folder).exists():
        QDir().mkpath(folder)
...
def copySessionData(filename, sessionFolderName):
    """
    Copies file to the user qgissessions/data folder to be used in the session

    :param filename: name of the data file to copy
    :param sessionFolderName: folder where the data file is stored (relative to
           the sessions group)
    """
    dest = os.path.join(sessionDataFolder(sessionFolderName), os.path.basename(filename))
...
def openProject(projectFile):
    ...
    tempDir = os.path.join(QDir.tempPath(), "sessions", "session{}".format(str(time.time())))
...
def openAttributeTable(layerName=None, typeName=None):
    ...
    # print('Opened {} attribute table'.format(iface.activeLayer().name()))
